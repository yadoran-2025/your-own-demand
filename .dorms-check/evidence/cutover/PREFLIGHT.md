...
- Preview deployment: `inflation-classroom-2tcsplapr-yadorans-projects.vercel.app` (`dpl_5a8YRT418E8bXG9mzZZrzMFUCLJD`), READY, commit `4a9aa15`.
- Firebase Auth authorized domains: verified exact Preview hostname `inflation-classroom-2tcsplapr-yadorans-projects.vercel.app` added through the Identity Toolkit configuration API; existing domains preserved.
...
- Browser verification: Chrome reached `/teacher`; `Google로 로그인` opened Firebase's Google popup with no CSP or Firebase error. Google profile/email consent was not accepted. `/student` in a separate regular tab rendered its ready room gate without console errors; `fetchSurveys` is now gated on an authenticated Firebase user in remote mode, so persisted-room data calls wait for anonymous bootstrap. This browser exposes no private-context capability, so isolated anonymous bootstrap remains unverified.
